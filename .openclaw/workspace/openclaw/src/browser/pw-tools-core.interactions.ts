...
/**
 * Validates a function body string before evaluation.
 * This is a basic security check - the code still runs in a browser sandbox.
 */
function validateFunctionBody(fnBody: string): void {
  if (typeof fnBody !== "string") {
    throw new Error("Function body must be a string");
  }
  if (fnBody.length > 10000) {
    throw new Error("Function body too large (max 10KB)");
  }
  // Basic syntax check - try to parse as a function
  try {
    // eslint-disable-next-line @typescript-eslint/no-implied-eval -- security validation only
    new Function(fnBody);
  } catch (e) {
    throw new Error(`Invalid function body: ${(e as Error).message}`);
  }
}
...
export async function evaluateViaPlaywright(opts: {
  ...
  // Validate function body before evaluation
