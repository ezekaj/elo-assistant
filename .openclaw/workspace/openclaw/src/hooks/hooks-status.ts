...
function resolveHookKey(entry: HookEntry): string {
  return entry.metadata?.hookKey ?? entry.hook?.name ?? entry.name;
}
...
function buildHookStatus(
  ...
  const hookConfig = resolveHookConfig(config as Record<string, unknown> | undefined, hookKey);
  const managedByPlugin = entry.hook?.source === "openclaw-plugin";
  const disabled = managedByPlugin ? false : (hookConfig as Record<string, unknown>)?.enabled === false;
  const always = entry.metadata?.always === true;
  const emoji = entry.metadata?.emoji ?? (entry.frontmatter?.emoji as string | undefined);
  const homepageRaw =
    entry.metadata?.homepage ??
    entry.frontmatter?.homepage ??
    entry.frontmatter?.website ??
    entry.frontmatter?.url;
  const homepage = typeof homepageRaw === 'string' && homepageRaw.trim() ? homepageRaw.trim() : undefined;
  ...
  for (const envName of requiredEnv) {
    ...
    const hookEnv = (hookConfig as Record<string, unknown>)?.env;
    if (hookEnv && typeof hookEnv === "object" && (hookEnv as Record<string, unknown>)[envName]) {
      continue;
    }
  ...
  return {
    name: entry.hook?.name ?? entry.name,
    description: entry.hook?.description ?? entry.description ?? "",
    source: entry.hook?.source ?? "openclaw-workspace",
